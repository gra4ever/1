...
from .parser import clean_text, key_text
from .parser_v04 import parse_program
...
def fetch_raw_program():
    doc = fetch_document()
    result = []
    tables = [x for x in doc.get("body", {}).get("content", []) if "table" in x]
    for table_index, element in enumerate(tables, 1):
        rows_out = []
...
def refresh_snapshot(source="automatic"):
    ...
    try:
        ...
        # Parser/schema upgrades should not create artificial News entries.
        migrating = bool(old_program is not None and old_status.get("version") != VERSION)
...
def api_program():
    snap = load_snapshot()
    status = load_status()
    if snap is not None and status.get("version") == VERSION:
        return snap
    try:
        return refresh_snapshot(source="migration")["program"]
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def api_refresh():
    try:
        ...
        return {
            ...
            "schema_migrated": result.get("schema_migrated", False),
...
def api_search(q: str = Query(..., min_length=1), limit: int = Query(50, ge=1, le=200)):
    ...
    if program is None or load_status().get("version") != VERSION:
        try:
            program = refresh_snapshot(source="migration")["program"]
        except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))
    results = []
    for month in program.get("months", []):
        ...
        events.extend(month.get("recitals", month.get("tmc_recitals", [])))
        for e in events:
